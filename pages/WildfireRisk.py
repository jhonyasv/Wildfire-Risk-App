...
def overlay_xarray_on_map(data_array):
    ...
    ax.set_xticks(np.linspace(data_array.lon.min(), data_array.lon.max(), 5), crs=ccrs.PlateCarree())
    ax.set_yticks(np.linspace(data_array.lat.min(), data_array.lat.max(), 5), crs=ccrs.PlateCarree())
...
# Model selection
model_choice = st.selectbox("Select a Model", options=["RF", "LSTM", "CNN"])
